import { r as reactExports, T as jsxRuntimeExports } from "./worker-entry-NTDgNiVY.js";
import "node:events";
...
const useMqttFleet = () => {
  const [fleet, setFleet] = reactExports.useState({});
...
function timeAgo(tsMs) {
  const now = Date.now();
  const seconds = Math.max(1, Math.round((now - tsMs) / 1e3));
...
function DeviceCard({ device, active, onSelect }) {
  const now = Date.now();
  const stale = now - device.ts_ms > 3e4;
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      type: "button",
      onClick: onSelect,
      className: `w-full rounded-lg border p-4 text-left transition hover:border-sidebar-ring ${active ? "border-sidebar-ring bg-sidebar-accent" : device.status === "crash" ? "border-emergency bg-emergency-muted" : "border-sidebar-border bg-sidebar"}`,
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `h-2.5 w-2.5 shrink-0 rounded-full ${statusDot[device.status]}` }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "truncate font-mono text-sm font-semibold text-sidebar-foreground", children: device.device })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: `mt-2 text-xs font-medium ${stale ? "text-warning" : "text-muted-foreground"}`, children: [
            "Last seen ",
            timeAgo(device.ts_ms)
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `rounded-full px-2.5 py-1 text-[11px] font-bold uppercase ${statusStyles[device.status]}`, children: device.status === "crash" ? "Crash" : "Online" })
      ] })
    }
  );
}
function SummaryField({ label, value, icon: Icon2 }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card p-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-muted-foreground", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-semibold uppercase tracking-[0.12em]", children: label })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-3 truncate text-lg font-semibold text-foreground", children: value })
  ] });
}
function CrashLocationDashboard() {
  const { fleet: fleetObj } = useMqttFleet();
  const fleet = Object.values(fleetObj).map((entry) => ({
  ...
  const priorityDevice = fleet.find((device) => device.status === "crash") ?? fleet[0];
  const [selectedDeviceId, setSelectedDeviceId] = reactExports.useState(priorityDevice?.device ?? "");
  const [search, setSearch] = reactExports.useState("");
  const filteredFleet = reactExports.useMemo(
    () => fleet.filter((device) => device.device.toLowerCase().includes(search.toLowerCase().trim())),
    [search, fleet]
  );
  const activeDevice = fleet.find((device) => device.device === selectedDeviceId) ?? priorityDevice;
  const incidentQueue = filteredFleet.filter((device) => device.status === "crash");
  const normalFleet = filteredFleet.filter((device) => device.status === "online");
  ...
  const mapSrc = activeDevice ? `https://www.openstreetmap.org/export/embed.html?bbox=${activeDevice.lon - 0.018}%2C${activeDevice.lat - 0.013}%2C${activeDevice.lon + 0.018}%2C${activeDevice.lat + 0.013}&layer=mapnik&marker=${activeDevice.lat}%2C${activeDevice.lon}` : "";
  if (!activeDevice) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "min-h-screen bg-background text-foreground flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-2xl font-bold mb-2", children: "No devices found" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground", children: "Waiting for device data from MQTT. Make sure your ESP is publishing to the correct topic." })
    ] }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "min-h-screen bg-background text-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mx-auto grid min-h-screen w-full max-w-390 lg:grid-cols-[360px_1fr]", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "border-b border-border bg-sidebar p-4 lg:border-b-0 lg:border-r lg:p-5", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex h-10 w-10 items-center justify-center rounded-lg bg-primary text-primary-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ShieldCheck, { className: "h-5 w-5", "aria-hidden": "true" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-semibold text-sidebar-foreground", children: "VestMicro" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Fleet response console" })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-xs font-semibold text-safe", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Radio, { className: "h-4 w-4", "aria-hidden": "true" }),
          "Live"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "mt-5 flex items-center gap-2 rounded-lg border border-sidebar-border bg-card px-3 py-2 text-sm text-muted-foreground", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mt-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3 flex items-center justify-between", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground", children: "Active incidents" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rounded-full bg-emergency px-2.5 py-1 text-xs font-bold text-emergency-foreground", children: incidentQueue.length })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-3", children: incidentQueue.map((device) => /* @__PURE__ */ jsxRuntimeExports.jsx(DeviceCard, { device, active: device.device === activeDevice.device, onSelect: () => setSelectedDeviceId(device.device) }, device.device)) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mt-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "mb-3 text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground", children: "Fleet devices" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-3", children: normalFleet.map((device) => /* @__PURE__ */ jsxRuntimeExports.jsx(DeviceCard, { device, active: device.device === activeDevice.device, onSelect: () => setSelectedDeviceId(device.device) }, device.device)) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "grid content-start gap-5 p-4 sm:p-6 lg:p-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            className: `mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold ${activeDevice.status === "crash" ? "bg-emergency-muted text-emergency" : "bg-safe-muted text-safe"}`,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "h-4 w-4", "aria-hidden": "true" }),
              "State: ",
              /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: currentState })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-3xl font-semibold text-foreground sm:text-4xl", children: "Incident Command" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "mt-2 max-w-2xl text-sm leading-6 text-muted-foreground", children: [
          activeDevice.device,
          " is selected for precise GPS review and location handoff."
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid gap-4 md:grid-cols-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(SummaryField, { label: "Device Identity", value: activeDevice.device, icon: ShieldCheck }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(SummaryField, { label: "Incident Status", value: currentState, icon: TriangleAlert }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(SummaryField, { label: "Precise Location", value: coordinates, icon: MapPin })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid gap-5 xl:grid-cols-[1fr_380px]", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: "overflow-hidden rounded-lg border border-border bg-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-xl font-semibold text-foreground", children: "Live location map" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1 font-mono text-sm text-muted-foreground", children: coordinates })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "a",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(Navigation, { className: "h-4 w-4", "aria-hidden": "true" }),
                  "Open in Google Maps"
                ]
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative min-h-90 bg-muted sm:aspect-video", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("iframe", { title: "Emergency GPS location", src: mapSrc, className: "h-full w-full border-0 grayscale-12", loading: "lazy" }, activeDevice.device),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pointer-events-none absolute left-4 top-4 rounded-lg border border-border bg-card/95 px-4 py-3 shadow-soft backdrop-blur", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `text-xs font-bold uppercase tracking-[0.14em] ${activeDevice.status === "crash" ? "text-emergency" : "text-safe"}`, children: "Map pin" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1 font-mono text-sm text-foreground", children: coordinates })
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: `rounded-lg border bg-card p-5 shadow-lift ${activeDevice.status === "crash" ? "border-emergency" : "border-border"}`, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `text-sm font-bold uppercase tracking-[0.14em] ${activeDevice.status === "crash" ? "text-emergency" : "text-safe"}`, children: "Location Handoff" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "mt-2 text-2xl font-semibold text-foreground", children: "Golden Action" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(MapPin, { className: `h-6 w-6 ${activeDevice.status === "crash" ? "text-emergency" : "text-safe"}`, "aria-hidden": "true" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("dl", { className: "mt-6 grid gap-3 text-sm", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg bg-secondary p-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { className: "text-muted-foreground", children: "Device ID" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("dd", { className: "mt-1 font-mono font-semibold text-foreground", children: activeDevice.device })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg bg-secondary p-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { className: "text-muted-foreground", children: "Coordinates" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("dd", { className: "mt-2 break-all font-mono text-2xl font-semibold text-foreground", children: coordinates })
            ] })
          ] }),
...
function Index() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(CrashLocationDashboard, {});
}
export {
  Index as component
};
